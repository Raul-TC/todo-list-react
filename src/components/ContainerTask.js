...
const ContainerTask = ({ modal, task, current, setModal, setcurrent, filteredData, updateTaskDone, handleDeleteTask, pendingTask, tasksDone, seTtask }) => {
  ...
  return (
    <>
      <div className={`${DarkTheme ? 'bg-container-task-dark' : 'bg-container-task'} max-w-85 w-full rounded-md h-auto  md:max-w-2xl`}>
      ...
      <div className={`${DarkTheme ? 'bg-container-task-dark' : 'bg-container-task'} w-full max-w-85 flex mt-4 py-4 items-center justify-center m-auto rounded-md  md:max-w-2xl  md:py-5 lg:hidden`}>
